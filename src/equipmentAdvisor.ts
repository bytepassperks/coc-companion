...
export function adviseEquipment(player: Player, goal = "balanced", ore?: { shiny?: number; glowy?: number; starry?: number }, lineup: string[] = [], loadouts: Record<string, { equipment: string[]; pet?: string }> = {}): EquipmentPlan {
  ...
  const equipmentAdvice = (player.heroes ?? []).flatMap((hero) => {
      const metaChoices = config.heroes[hero.name]?.[goal] ?? config.heroes[hero.name]?.balanced;
      if (!metaChoices) return [];
      const selected = loadouts[hero.name]?.equipment?.slice(0, 2) ?? [];
      const choices = selected.length ? selected : metaChoices;
      ...
      return [{
        ...
        metaSuggestion: selected.length && selected.join("|") !== metaChoices.join("|") ? metaChoices : undefined,
        priority: target
  ...
  const pets = petsPayload.length
    ? petsPayload.map((pet) => {
      ...
      const selectedPet = loadouts[pair?.[0] ?? ""]?.pet;
      const hero = pair?.[0];
      return {
        name: pet.name,
        level: pet.level,
        ...
        priority: selectedPet === pet.name
          ? `${pet.name} is your selected pet for ${hero}; schedule its next upgrade by its roughly ${config.petUpgradeTimeHours[pet.name] ?? 48}h laboratory time.`
          : hero ? `${pet.name} pairs with ${hero}; next upgrade should be scheduled by its roughly ${config.petUpgradeTimeHours[pet.name] ?? 48}h laboratory time.` : `${pet.name} is not in the curated pairing list; treat as a lower-confidence option.`,
        provenance: `equipment-meta v${config.version}, dated ${config.dated}; ${config.sourceNote}`,
      };
    })
    : [];
